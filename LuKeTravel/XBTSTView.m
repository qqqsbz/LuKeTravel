...
- (void)updateBasicSubviewsConstraint {
    ...
    if ([self.delegate respondsToSelector:@selector(hiddenTabSeparator)]) {
        self.tabSeparator.hidden = [self.delegate hiddenTabSeparator];
    }
    ...
    [self.tabSeparator makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.right.equalTo(self.topTabView);
...
- (void)addConstraintToTabButtons
{
    ...
    NSInteger btnCount = self.tabButtons.count;
    if (btnCount <= self.onePageOfItemCount) {
        self.topTabView.contentSize = self.topTabView.frame.size;
        self.topTabView.scrollEnabled = NO;
        CGFloat width = CGRectGetWidth(self.topTabView.frame);
        self.leading  = self.zeroMargin ? 0 : self.leading;
        width = (width - self.leading * (btnCount + 1)) / btnCount;
        [firstButton makeConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(self.topTabView).offset(self.leading);
        ...
        [lastButton makeConstraints:^(MASConstraintMaker *make) {
            make.right.equalTo(self.topTabView).offset(self.leading);
        ...
        if (btnCount == 2) {
            [lastButton updateConstraints:^(MASConstraintMaker *make) {
                make.left.equalTo(firstButton.right).offset(self.leading);
            }];
            return;
        }
        
        for (NSInteger i = btnCount - 1; i >= 1 ; i --) {
            UIButton *currentBtn  = self.tabButtons[i];
            UIButton *previousBtn = self.tabButtons[i - 1];
            [currentBtn makeConstraints:^(MASConstraintMaker *make) {
                make.left.equalTo(previousBtn.right).offset(self.leading);
    ...
    [firstButton makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.topTabView).offset(self.leading * 2);
    }];
    [lastButton makeConstraints:^(MASConstraintMaker *make) {
    ...
    for (NSInteger i = btnCount - 1; i > 0; --i) {
...
{
    if (page < self.tabButtons.count) {
        [self selectTab:self.tabButtons[page]];
    }
}
